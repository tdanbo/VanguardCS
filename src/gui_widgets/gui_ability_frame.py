...
class AbilityItem(QWidget):
    def __init__(self, character, ability_dict, select=False, slot=None):
        ...
        category_text = self.restyle_description(
            ", ".join([self.ability_dict["Type"], self.ability_dict["Tradition"]])
        )
        if self.ability_dict["Type"] in ["Ritual", "Mystical Power"]:
            category_text = self.restyle_description(
                ", ".join(
                    [
                        self.ability_dict["Type"],
                        self.ability_dict["Tradition"],
                        "1 Permanent Corruption",
                    ]
                )
            )
        ...
        self.build_dice_section(
            self.ability_dict["Novice"], self.novice_section.inner_layout(2)
        )
        ...
        self.build_dice_section(
            self.ability_dict["Adept"], self.adept_section.inner_layout(2)
        )
        ...
        self.build_dice_section(
            self.ability_dict["Master"], self.master_section.inner_layout(2)
        )
        ...
        self.setStyleSheet(
            f"border-style: outset; color: {cons.FONT_DARK}; background-color: {cons.PRIMARY}; border-style: outset;"
        )
    ...
    def gui_rank_state(self):
        if self.ability_dict["Novice"] == "":
            ...
            self.set_rank_novice.get_widget().setStyleSheet(
                f"background-color: {self.type_bg_color}; color: {cons.PRIMARY_LIGHTER}; font-size: {cons.FONT_SMALL}; font-weight: bold; border: 1px solid {cons.FONT_MEDIUM}; border-radius: 6px;"
            )
            self.set_rank_adept.get_widget().setStyleSheet(
                f"background-color: {cons.DARK}; color: {cons.DARK}; font-size: {cons.FONT_SMALL}; border: 1px solid {cons.DARK}; border-radius: 6px;"
            )
            self.set_rank_master.get_widget().setStyleSheet(
                f"background-color: {cons.DARK}; color: {cons.DARK}; font-size: {cons.FONT_SMALL}; border: 1px solid {cons.DARK}; border-radius: 6px;"
            )
        ...
        if self.ability_dict["Rank"] == "Novice":
            ...
            self.set_rank_adept.get_widget().setStyleSheet(
                f"background-color: {cons.DARK}; color: {cons.PRIMARY_LIGHTER}; font-size: {cons.FONT_SMALL}; border: 1px solid {cons.FONT_MEDIUM}; border-radius: 6px;"
            )
            self.set_rank_master.get_widget().setStyleSheet(
                f"background-color: {cons.DARK}; color: {cons.PRIMARY_LIGHTER}; font-size: {cons.FONT_SMALL}; border: 1px solid {cons.FONT_MEDIUM}; border-radius: 6px;"
            )
